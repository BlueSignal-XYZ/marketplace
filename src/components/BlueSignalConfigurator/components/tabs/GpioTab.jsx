      ...
      <div style={{ marginTop: 24 }}>
        ...
        <div style={{ overflowX: 'auto' }}>
          <Table>
            ...
            <tbody>
              {GPIO_PINOUT.filter((p) => p.type !== 'ground' || p.pin <= 20)
                .slice(0, 20)
                .map((pin) => (
                  <tr key={pin.pin}>
                    <Td>{pin.pin}</Td>
                    ...
                    <Td style={{ textAlign: 'left', color: '#94a3b8', fontSize: 11 }}>
                      {pin.notes}
                    </Td>
                  </tr>
                ))}
        ...
        <p style={{ fontSize: 11, color: '#64748b', marginTop: 8 }}>
          Showing first 20 pins. Full pinout available in documentation. Reserved pins (UART, SPI,
          EEPROM) should not be used.
        </p>
      </div>
    </div>
  );
