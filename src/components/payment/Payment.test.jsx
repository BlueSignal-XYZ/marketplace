...
vi.mock('./CheckoutForm', () => ({
  default: ({ item }) => (
    <div data-testid="checkout-form">CheckoutForm - Item: {item?.item?.name || 'no-item'}</div>
  ),
}));
...
vi.mock('./OrderConfirmation', () => ({
  default: ({ proceedToPayment, checkoutItems }) => (
    <div data-testid="order-confirmation">
      OrderConfirmation
      <button onClick={() => proceedToPayment({ name: 'Test Item', price: 100 }, 100)}>
        Proceed to Payment
      </button>
...
describe('Payment Component', () => {
  ...
  describe('Rendering', () => {
    it('should render the payment form wrapper', () => {
      render(<Payment setIsPaying={mockSetIsPaying} checkoutItems={mockCheckoutItems} />);
