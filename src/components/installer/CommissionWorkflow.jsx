...
const CommissionWorkflow = () => {
  ...
  return (
    <PageContainer>
      <PageHeader>
        <BackLink onClick={() => navigate('/commissions')}>← Back to Commissions</BackLink>
              ...
              <TestGrid>
                {commission.testResults?.map((test) => (
                  <TestItem key={test.id}>
                    ...
                    <TestStatus status={test.status}>{test.status.toUpperCase()}</TestStatus>
                  </TestItem>
                ))}
              </TestGrid>
            </CardBody>
      ...
      <ButtonGroup>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {currentStep === STEPS[STEPS.length - 1].id ? (
            <Button variant="success" onClick={handleComplete} disabled={saving || !canComplete()}>
