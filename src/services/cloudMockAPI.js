  ...
  'pgw-0002': {
    ...
    tests: [
      ...
      },
      {
        ...
        status: 'failed',
        duration: 2100,
        details: 'Timeout on register read',
      },
...
export const CloudMockAPI = {
  ...
  commissioning: {
    ...
    runCommission: async (deviceId, onTestUpdate = null) => {
      ...
      for (let i = 0; i < tests.length; i++) {
        ...
        const shouldPass =
          device.commissionStatus === 'commissioned' ||
  ...
  overview: {
    getStats: async () => {
      ...
      const devicesNeedingAttention = MOCK_DEVICES.filter((d) => d.status !== 'online').length;
